...
- **合成** — W3C準拠のブレンド + ストレートアルファ合成（CPU、テスト可能）。
  レイヤーマスク乗算とグループの独立合成に対応。GL 環境では WebGL2 合成パスを使用し、
  非対応環境では CPU 合成へ自動フォールバック(`compositeAuto`)。
- **フィルター** — ガウシアンぼかし / 明るさ・コントラスト / 階調反転 / グレースケール /
  色相・彩度 / レベル補正 / シャープ / しきい値 / ポスタライズ / セピア。
  選択範囲を尊重し、Undo 可能（純粋関数・テスト可能）。
- **ツール** — ブラシ / 消しゴム / 塗りつぶし(許容値付き flood fill) / スポイト /
  矩形選択 / 投げ縄選択 / 移動 / 変形 / テキスト(組み込み5x7ビットマップフォント) / 手のひら(パン)。
...
- **書き出し (`exportCLIP`)** — 有効な SQLite コンテナに**独自スキーマ**(`hitpaint_meta` /
  ...
  **HIT Paint で完全に再読込(往復)できます**が、**本物の CLIP STUDIO PAINT では開けません**
  （CSP 互換の `.clip` 生成は本アプリの対象外）。旧スキーマ(mask 列なし)も後方互換で読み込めます。
...
- グループ(フォルダ)は合成・モデル・CLIP 往復に対応済みだが、UI 上のドラッグでの編成
  (ネスト構造の自由な組み替え)は簡易版。テキストは組み込みビットマップフォントによる
  ラスター描画ツールとして実装(再編集可能なテキストレイヤー/任意 TTF・ベクターは未対応)。
  `.clip` の CSP 互換書き出しは対象外。フィルターは破壊適用のみ(調整レイヤー化は未対応)。
